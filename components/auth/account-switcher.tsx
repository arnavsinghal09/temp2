...
import { UserManager, DUMMY_CREDENTIALS, type User } from "@/lib/user-management"
...
export function AccountSwitcher({ currentUser, onUserChange }: AccountSwitcherProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const user = UserManager.authenticateUser(loginForm.email, loginForm.password)
    if (user) {
      onUserChange(user)
      setShowLogin(false)
      setIsOpen(false)
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {isOpen && (
          <div className="absolute right-0 top-12 w-80 bg-gray-950 border border-gray-800 rounded-lg shadow-2xl z-50 p-4">
            {!showLogin ? (
              <div>
                <h3 className="text-[#ff6404] font-semibold mb-4">Quick Account Switch</h3>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="w-8 h-8 bg-gradient-to-br from-[#ff6404] to-orange-600 rounded-full flex items-center justify-center">
          <span className="text-black font-bold text-sm">{currentUser.name.charAt(0)}</span>
        </div>
        <div className="text-left">
        ...
        <div className="absolute right-0 top-12 w-80 bg-gray-950 border border-gray-800 rounded-lg shadow-2xl z-50 p-4">
          <div className="flex items-center space-x-3 mb-4 pb-4 border-b border-gray-800">
            <div className="w-12 h-12 bg-gradient-to-br from-[#ff6404] to-orange-600 rounded-full flex items-center justify-center">
              <span className="text-black font-bold text-lg">{currentUser.name.charAt(0)}</span>
            </div>
            <div>
          ...
          <div className="space-y-2 mb-4">
            <h4 className="text-gray-400 text-sm font-medium">Switch Account</h4>
            {DUMMY_CREDENTIALS.filter((cred) => cred.userId !== currentUser.id)
              .slice(0, 3)
              .map((cred) => {
                const user = UserManager.getUserById(cred.userId)
                return (
